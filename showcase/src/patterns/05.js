...
const MediumClap = ({
  ...
  useEffect(() => {
    if (!componentJustMounted.current && !isControlled && onClap) {
      onClap(clapState);
    }
  ...
  const handleClapClick = () => {
    ...
    if (isControlled) {
      ...
      setClapState((prev) => ({
        isClicked: true,
        count: Math.min(MAXIMUM_USER_CLAP, prev.count + 1),
        countTotal:
...
const Usage = () => {
  ...
  const handleClap = () => {
    setState((prev) => ({
      ...prev,
      count: Math.min(prev.count + 1, MAXIMUM_CLAP_VAL),
      countTotal:
        prev.count < MAXIMUM_CLAP_VAL ? prev.countTotal + 1 : prev.countTotal,
      isClicked: true,
    }));
  };
